...
async def _try_login(
    ...
    for app_type in (TUYA_SMART_APP, SMARTLIFE_APP, ""):
        ...
        response = await manager.login_with_credentials(data, add_to_cache=True)
...
def _show_login_form(
    flow_handler: TuyaBLEConfigFlow | TuyaBLEOptionsFlow,
    user_input: dict[str, Any],
    errors: dict[str, str],
    placeholders: dict[str, Any],
) -> ConfigFlowResult:
    _LOGGER.debug(
        "Showing login form with user input: %s, errors: %s", user_input, errors
    )
    ...
    try:
        def_country = pycountry.countries.get(alpha_2=flow_handler.hass.config.country)
        if def_country:
            def_country_name = def_country.name
    except AttributeError:
        msg = "Failed to get default country"
        raise RuntimeError(msg) from None

    return flow_handler.async_show_form(
        step_id="login",
        data_schema=vol.Schema(
            {
                vol.Required(
                    ...
                    # We don't pass a dict {code:name} because country codes
                    # can be duplicate.
...
class TuyaBLEOptionsFlow(OptionsFlowWithConfigEntry):
    ...
    def __init__(self, config_entry: ConfigEntry) -> None:
        ...
        _LOGGER.debug("Initializing TuyaBLEOptionsFlow")
    ...
    async def async_step_login(
        ...
        _LOGGER.debug("Starting options login step with input: %s", user_input)
        errors: dict[str, str] = {}
        placeholders: dict[str, Any] = {}
...
class TuyaBLEConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    def __init__(self) -> None:
        ...
        super().__init__()
        _LOGGER.debug("Initializing TuyaBLEConfigFlow")
    ...
    async def async_step_bluetooth(
        ...
        _LOGGER.debug(
            "Starting Bluetooth discovery step with device: %s", discovery_info.address
        )
    ...
    async def async_step_user(
        ...
        """Handle the user step for manual configuration of Tuya BLE integration."""
        _LOGGER.debug("Starting user initiated config flow with input: %s", user_input)
        if self._manager is None:
            self._manager = HASSTuyaBLEDeviceManager(self.hass, self._data)
        await self._manager.build_cache()
        return await self.async_step_login()
    ...
    async def async_step_login(
        ...
        """Handle the Tuya IOT platform login step."""
        _LOGGER.debug("Starting login step with input: %s", user_input)
        data: dict[str, Any] | None = None
        errors: dict[str, str] = {}
        placeholders: dict[str, Any] = {}

        if self._manager is None:
            msg = "Device manager is not initialized"
        ...
        if user_input is not None:
            ...
            if data:
                self._data.update(data)  # save data for future use
                _LOGGER.debug("Login successful, proceeding to device selection")
                return await self.async_step_device()
        else:
            _LOGGER.debug(
                "No user input provided, attempting to load credentials from cache"
            )
            user_input = {}
    ...
    async def async_step_device(
        self, device_selected_user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Handle the user step to pick discovered device."""
        _LOGGER.debug(
            "Starting device selection step with input: %s", device_selected_user_input
        )
        ...
        if device_selected_user_input is not None:
            _LOGGER.debug(
                "Device selected with user input: %s", device_selected_user_input
            )
            address = device_selected_user_input[CONF_ADDRESS]
        ...
        def_address: str
        if (
            device_selected_user_input is not None
            and CONF_ADDRESS in device_selected_user_input
        ):
            def_address = device_selected_user_input.get(CONF_ADDRESS) or ""
        else:
        ...
        _LOGGER.debug(
            ...
            {
                addr: await get_device_readable_name(info, self._manager)
